...
void Tester::find(const vector<int> &keys, const vector<string> &search,
				  int &error) {
	int found = -1;
	error = 0;
	vector<int>::const_iterator ivid;
...
void Tester::confirmAbsent(const vector<int> &keys, const string *search,
						   int &error) {
	int found = -1;
	error = 0;
	vector<int>::const_iterator ivid;
...
void Tester::confirmAbsent(const vector<int> &keys,
						   const vector<string> &search, int &error) {
	int found = -1;
	error = 0;
	vector<int>::const_iterator ivid;
	for (ivid = keys.begin(); ivid != keys.end(); ++ivid) {
...
void Tester::findInOrder(const vector<int> &keys, const string *search,
						 int &error) {
	int found = -1;
	error = 0;
	vector<int>::const_iterator ivid;
...
void Tester::findInOrder(const vector<int> &keys, const vector<string> &search,
						 int &error) {
	int found = -1;
	error = 0;
	vector<int>::const_iterator ivid;
...
void Tester::initCapture() {
	cout << "inside initCapture" << endl;
	oldCoutStreamBuf = std::cout.rdbuf();
	cout << "initCapture A" << endl;
	ostringstream temp;
	cout << "initCapture B" << endl;
	strCout.swap(temp);
	cout << "initCapture C" << endl;
	std::cout.rdbuf(strCout.rdbuf());
	cout << "ending initCapture" << endl;
}
...
// generate and return a single random number in the range range_from to
// range_to - 1
